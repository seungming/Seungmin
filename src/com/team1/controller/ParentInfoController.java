...
public class ParentInfoController
{
    ...
    @RequestMapping(value = "/parentinfoview.action")
    public String parentInfoView(HttpSession session, Model model)
    {
        // 로그인한 부모의 백업 ID 가져오기
        String par_backup_id = (String) session.getAttribute("par_backup_id");
        
        if (par_backup_id == null || par_backup_id.trim().equals(""))
		{
			// 세션에 값이 없다면 로그인 페이지 등으로 리다이렉트
        	return "redirect:/login.action";
		}

        // DAO로부터 부모 정보 조회
        IParDAO dao = sqlSession.getMapper(IParDAO.class);
        ParDTO parent = dao.search(par_backup_id);
